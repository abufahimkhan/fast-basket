        ...
        {navItems.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className="relative flex flex-col items-center justify-center flex-1 gap-1 group"
            >
              {/* Icon */}
              ...
              {/* Active Indicator */}
              {isActive && (
                <div className="absolute top-1 left-1/2 -translate-x-1/2 w-6 sm:w-8 h-0.5 bg-orange-500 rounded-full" />
              )}
            </Link>
          );
        })}
